...
void w2(){
	FILE * f = fopen("text1.txt", "at"); // at모드 : append + text (기존 텍스트 파일에 내용을 추가한다. 파일에 없으면 새로 만듦 ) 
...
void w3(){
	...
	//fprintf() 함수, v파일을 대상으로 printf() 사용  
...
void r(){
	int ch;
	FILE * f = fopen("text1.txt", "rt"); // rt 모드 : read + text (텍스트 파일 읽기)
