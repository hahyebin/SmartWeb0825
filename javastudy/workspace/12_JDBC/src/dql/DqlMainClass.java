...
public class DqlMainClass {
	...
	public static void selectOne() {
		...
		// 필요한 정보
		 ...
		 ResultSet rs = null;     // select 문의 실행 결과
		 String sql = null;    
		 ...
		 try {
			 ...
			 //    검색 결과가 1개가 될 수 있도록 
			 //    WHERE절에서 PK를 사용
			 sql = "SELECT PNO, PNAME, PRICE, PDATE FROM PRODUCT WHERE PNO=?";
	...
	public static void selectValue() {
		...
		try {
			...
			if( rs.next() ) {
				...
				// |    2     | 
				// ------------
			//long rowCnt = rs.getInt(1);	
			long rowCnt = rs.getInt("COUNT(*)");
			   if( rowCnt > 0) {
			       System.out.println("현재 제품이 " + rowCnt +"개 있습니다."); //1번째 칼럼 정보 가져오기
			   } else {
				   System.out.println("저장된 제품이 없습니다.");
			  }
		  } else {
				//  PRODUCT 테이블이 존재한다면 else문이 동작하지 않는다.
				System.out.println("PRODUCT 테이블이 아닙니다.");
			}
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void selectList() {
		...
		try {
			...
			// ProductDTO 객체를 저장할 ArrayList에 생성
			...
			while( rs.next() ) {
				...
				// 결과 행(row)은 모두 ProductDTO 객체로 저장한다/
				ProductDTO product = new ProductDTO();
				product.setpNo(rs.getLong(1));
				product.setpName(rs.getString(2));
				product.setPrice(rs.getInt(3));
				product.setpDate(rs.getDate(4));   // --> 한줄씩 갖고와서 ArrayList에 모음
