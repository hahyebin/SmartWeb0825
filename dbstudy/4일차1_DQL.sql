-- DQL
-- 1. employees 테이블에서 first_name, last_name 을 조회하시오
...
-- 3. employees 테이블에서 employee_id 가 150인 사원의 employee_id,first_name, last_name 을 조회하시오.
...
SELECT employee_id, first_name, last_name 
  FROM employees 
 WHERE employee_id='150';     -- WHERE employee_id = TO_NUMBER('150') 으로 처리된다.(오라클의 내장함수)  -- 문제 없음
...
SELECT employee_id, first_name, last_name 
  FROM employees 
 WHERE employee_id=150;   -- WHERE TO_NUMBER(employee_id) = 150; 으로 처리된다.
                          -- employee_id는 PK이다. PK로 사용되는 칼럼을 함수로 처리하면 속도가 떨어진다.  (pk칼럼은 성능 속도 저하의 문제로 인해 가급적 함수로 사용하지 않는것이 좋다)
...
SELECT employee_id
     , NVL(commission_pct*1.1,0) as "10% 인상 commission"
     , NVL2(commission_pct,commission_pct*1.1, 0) as "10% 인상 commission"
  FROM employees;
...
-- SELECT * FROM employees ORDER BY employee_id, salary DESC;   -- employee_id는 ASC 생략으로 오름차순, salary DESC;


-- 11. employees 테이블에서 commission_pct가 없는 사원들을 높은 salary순으로 조회하시오.
SELECT *
  FROM employees
 WHERE commission_pct IS NULL
...
SELECT e.first_name, e.last_name, e.department_id, d.department_name, e.salary
...
-- 서브쿼리 preview 
-- 하위 쿼리 : 쿼리 내부에 포함된 쿼리(SELECT 안에 SELECT)
