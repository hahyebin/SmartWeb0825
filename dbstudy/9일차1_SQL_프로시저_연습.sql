...
-- INSERT
INSERT INTO books(BOOK_ID, BOOK_NAME, PUBLISHER, PRICE) VALUES(BOOKS_SEQ.NEXTVAL, '축구의 역사', '굿스포츠', 7000 );
...
--      order_id, customer_name, book_name, sales_price, order_date를 조회하시오. (맞춤)
SELECT  o.order_id, c.customer_name, b.book_name, o.sales_price, o.order_date
 ...
 WHERE b.book_id = o.book_id 
   ...
   AND o.order_date BETWEEN '21/09/04' AND'21/09/10';
...
-- 2. '손흥민'의 총 구매액을 조회하시오. (맞춤)
-- 1) 조인
SELECT C.CUSTOMER_NAME, SUM(sales_price) AS 총구매액
...
SELECT C.CUSTOMER_NAME, SUM(sales_price) AS 총구매액
 ...
 WHERE c.customer_id = o.customer_id
   AND c.customer_name = '손흥민'
 GROUP BY c.customer_id, c.customer_name;   --GROUP BY  c.customer_name 일때는 동명이인 사람을 묶어버리기 때문에 customer_id와 함께 묶어서 id와 name을 함께 묶어 식별함

-- 2) 서브쿼리(인라인 뷰 : FROM절의 서브쿼리)
SELECT a.customer_name,  sum(a.sales_price)
...
-- 3. '김연아'가 구매한 도서의 개수를 조회하시오.    (맞춤)
SELECT c.customer_name, COUNT(o.customer_id) AS 구매도서수
  FROM orders o, customers c
 WHERE c.customer_id = o.customer_id
...
-- 4. '추신수'가 구매한 도서들의 출판사수를 조회하시오.   (맞춤)
SELECT c.customer_name, b.publisher, COUNT(b.publisher) AS 출판사수
  FROM orders o, customers c, books b
 WHERE c.customer_id = o.customer_id
   AND o.book_id = b.book_id   
   AND c.customer_name = '추신수'
 GROUP BY c.customer_id, c.customer_name, b.publisher;

-- 답 
SELECT c.customer_name, COUNT(DISTINCT b.publisher) AS 출판사수
  FROM orders o, customers c, books b
 WHERE c.customer_id = o.customer_id
   AND o.book_id = b.book_id   
   AND c.customer_name = '추신수'
...
-- 5. 고객별 총 구매액을 조회하시오.
--  customer_id, customer_name, 총 구매액을 조회하시오.   (맞춤)
...
-- 6. 주문한 이력이 없는 고객의 customer_name을 조회하시오.   (맞춤)
SELECT c.customer_name
...
-- 7.  고객별 총 구매횟수를 조회하시오.
--    customer_id, customer_name, 총구매횟수를 조회하시오.
--    구매한 적이 없으면 0으로 조회하시오.   (모든 고객은 포함하는 외부 조인)      (맞춤)                    
SELECT c.customer_id, c.customer_name, NVL(COUNT(o.customer_id),0) AS 총구매횟수
  FROM customers c LEFT JOIN orders o
    ON c.customer_id = o.customer_id
 GROUP BY  c.customer_id, c.customer_name;
...
-- 답 
SELECT c.customer_id, c.customer_name, COUNT(o.customer_id) AS 총구매횟수
  FROM customers c, orders o
 WHERE c.customer_id = o.customer_id(+)
 GROUP BY  c.customer_id, c.customer_name;
...
-- 1. BOOKS 테이블에 재고(STOCK) 칼럼을 추가혹 기존의 모든 책의 재고를 10개로 세팅하시오.
...
-- 3. ORDERS 테이블에 구매수량(AMOUNT) 칼럼을 추가하고 기본의 모든 구매 내역의 구매 수량을 1로 세팅하시오
...
-- 5. 모두 성공하면 COMMIT, 중간에 실패하면 ROLLBACK 한다.
...
BEGIN
   INSERT INTO ORDERS (ORDER_ID, CUSTOMER_ID, BOOK_ID, SALES_PRICE, AMOUNT, ORDER_DATE) 
     VALUES (ORDERS_SEQ.NEXTVAL
           ...
           , V_AMOUNT
           , SYSDATE);                                                                       -- WHERE ORDER_ID = ? 방금 들어온 주문 번호입력하기     
