-- SYSTEM 계정에서 SCOTT에게 권한주기(GRANT DBA TO SCOTT)
-- HR 계정에 있는 employees 테이블을 SCOTT계정으로 복사하기
...
-- 3. 변수 선언하기(참조 변수) -- 데이터에 있는 값 가져오기
--    테이블.칼럼%TYPE
DECLARE 
...
-- 연습문제. employee_id가 200인 사원의 salary가 5000이상이면 '고액연봉' 아니면 공백을 출력하시오.
DECLARE 
   v_employee_id employees.employee_id%TYPE;
   v_salary employees.salary%TYPE;
...
BEGIN 
   ...
   ELSE 
       sal := ' ';
   END IF;
   DBMS_OUTPUT.PUT_LINE('사원번호 : ' || v_employee_id || '인 직원은' || v_result || '이다.');
END;
...
DECLARE 
   ...
   v_employee_id employees.employee_id%TYPE;
BEGIN 
   SELECT salary, employee_id INTO v_salary, v_employee_id
     FROM employees 
   ...
   ELSE 
        v_result := ' ';
   END IF;
    DBMS_OUTPUT.PUT_LINE('직원아이디: '|| v_employee_id || ', 연봉 : ' || v_salary || ', 결과: ' || v_result);
END;
...
BEGIN
   ...
   WHILE num < 101 LOOP 
        ...
        num := num+1;
   END LOOP;
   DBMS_OUTPUT.PUT_LINE('1~100 사이의 모든 정수의 합 : ' || total);
END;
...
BEGIN 
   ...
   FOR V_employee_id IN 100..206 LOOP
     total := total +  v_salary ;
      
     END LOOP;
     DBMS_OUTPUT.PUT_LINE('월급의 합 : '|| total);
END;  
...
-- 11. 변수 선언(레코드 타입)
-- 특정 테이블의 레코드 전체 타입이 자동으로 세팅 
DECLARE
   -- 레코드 타입 변수 선언 --> 행 전체 갖고 올 수 있음 
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('연봉 : ' || v_row.salary );
END;



-- 연습문제, department_id가 50인 사원들의 정보를 dept50 테이블을 생성하시오. 
CREATE TABLE dept50 
...
-- 12. 변수 선언(테이블 타입)
--  특정 칼럼의 데이터들은 배열처럼 여러데이터를 저장하는 구조로 사용
DECLARE 
