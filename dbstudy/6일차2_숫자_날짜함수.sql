...
-- 7. 내림                                  -- 올림과 내림은 ,로 두개 지정하는 것이 안된다. 인수는 한개만!
...
-- 8. 절사 
--    TRUNCATE TABLE : DDL (CREATE / ALTER / DROP) 테이블 삭제, 테이블의 구조는 남기고 행만 모두 삭제(잘라 버린다.)
...
SELECT TRUNC(1.9), TRUNC(1.99,1), TRUNC(1.999, 2), TRUNC(999.999,-1), TRUNC(999.999, -2), TRUNC(999.999, -3)
  FROM DUAL;
...
SELECT CEIL(1.111 * 100) / 100     -- 10 : 10의 2제곱 
  FROM DUAL;
...
SELECT CEIL(1.11 * POWER(10, 1)) / POWER(10, 1)
  FROM DUAL;
...
SELECT SYSDATE + 1 AS 내일 
     ...
     , TO_CHAR(SYSDATE + 0.5, 'YYYY/MM/DD HH:MI:SS') AS "12시간후"       -- 날짜 형식은 TO_CHAR
...
SELECT ADD_MONTHS(hire_date,12) AS "1년후"
     , ADD_MONTHS(hire_date, -12) AS "1년전"
  FROM employee;
...
SELECT name AS 사원명
     ...
     ,CEIL(MONTHS_BETWEEN(SYSDATE, hire_date)) AS "근무개월수(올림)"
     ,FLOOR(MONTHS_BETWEEN(SYSDATE, hire_date)) AS "근무개월수(내림)"
  FROM employee;


SELECT CEIL(SYSDATE-HIRE_DATE)
  FROM EMPLOYEE; 
