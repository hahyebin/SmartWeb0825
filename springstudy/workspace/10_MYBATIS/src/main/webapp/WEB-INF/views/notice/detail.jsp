<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core"  prefix="c" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<script src="https://code.jquery.com/jquery-3.6.0.js" integrity="sha256-H+K7U5CnXl1h5ywQfKtSj8PCmoN9aaq30gDh27Xc0jk=" crossorigin="anonymous"></script>
<title>Insert title here</title>
<script>
   $(document).ready(function(){
	   // 수정 
	   $("#update_btn").on("click",function(){
		    if( '${notice.title}' == $("#title").val() && '${notice.content}' == $("#content").val() ) {
		    	alert("수정할 사항이 없습니다.");
		    	return;
		    }
		   
			$("#f").attr('action',"/ex10/notice/updateNotice");
			$("#f").submit();
		})
		
		$("#delete_btn").on("click",function(){
			if(confirm("삭제할까요?")){
				$("#f").attr('action',"/ex10/notice/deleteNotice");
				$("#f").submit();
			}
		})
   })
	</script>
</head>
<body>
	
	<h1>공지사항 상세보기 및 수정</h1>
	<form id="f">
		<strong>번호</strong> <br>
		${notice.no}<br><br>
		<strong>제목</strong> <br>
		<input type="text" name="title" id="title" value="${notice.title}"><br><br>
		<strong>내용</strong><br>
		<textarea rows="2" cols="22" name="content" id="content">${notice.content}</textarea><br><br>
		<strong>작성일</strong><br>
		${notice.created}<br><br>
		<strong>최종수정일</strong><br>
		${notice.lastModified}<br><br>
		<input type="hidden" name="no" value="${notice.no}">           <!--  게시글의 번호를 알아야 가능 -->
		<input type="button" value="수정" id="update_btn"  >
		<input type="button" value="삭제" id="delete_btn"  >
		<input type="button" value="목록" onclick="location.href='/ex10/notice/selectNoticeList'" >
	</form>
	
</body>
</html>