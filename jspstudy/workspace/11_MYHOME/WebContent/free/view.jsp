...
<body>
	<div>
		...
		<span>게시글 번호</span> : ${free.fNo}<br>  <!-- = requestScope.free.nNo -->
		<span>작성자</span> : ${free.writer}<br>
		<span>조회수</span> : ${free.hit}<br>
		<span>IP</span> : ${free.ip}<br>
		<span>작성일</span> : ${free.created}<br>
		<span>최종수정일</span> : ${free.lastModified}<br>
		내용<br>
	 ...
	 -->
		...
		<script>
			function fnFreeUpdate(){
		 		$('#f').attr('action', 'updateForm.free');   // action 속성 추가
		 		$('#f').submit();							 // 서브밋하기(번호와 내용 담겨있음)
			}
