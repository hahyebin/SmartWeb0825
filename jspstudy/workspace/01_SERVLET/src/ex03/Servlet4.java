...
public class Servlet4 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if (optName.isPresent()) {    // optName으로 감싼(wrapper) 데이터가 있으면 
			System.out.println("이름: "+optName.get());			
		}
		*/
		
		Optional<String> optName = Optional.ofNullable(request.getParameter("name"));
		String name = optName.orElse("혜빈"); // optName으로 감싼 데이터가  null이면 "혜빈" 사용 
		
		Optional<String> optAge = Optional.ofNullable(request.getParameter("age"));
		int age = Integer.parseInt(optAge.orElse("25"));
		
		
	
//		Optional<Integer> age = Optional.ofNullable( Integer.parseInt(request.getParameter("age")));
