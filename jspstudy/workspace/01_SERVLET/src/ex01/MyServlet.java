 ...
 * javadoc : 주석이 문서화가 됨.(**)
 * 
 * - servlet 실행 -> 웹 서버를 통해서 웹 브라우저가 실행됨.
 * - URL 구성 -> http://localhost:9090/01_SERVLET/MyServlet  ====  http://localhost:9090/ContextPath/URLMapping
 */
...
public class MyServlet extends HttpServlet {
	 ...
	 * 2. init 메소드
	 *     1) 최초 1번만 실행된다.
	 *     2) 환경 설정(Config) 용도로 사용한다.  (환경설정 전달)
	 ...
	 *     -- service() 에서 서비스 처리하면 doGet(). doPost()가 실행하지않는다-> 앞으로 수업내용에선 service()는 이용하지 않음
	 */
	 ...
	 *    	(1) URL에 포함된 파라미터(http://호스트:포트/ContextPath/URLMapping?parameter1=value1&parameter2=value2)
	 *    	(2) <form method="get">
	 ...
	 *    4) service() 메소드가 없으면 실제 업무 처리를 doGet()이 담당한다.
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1) 사용자에게 보여 줄 응답 형식 지정 (HTML 문서로 응답)
